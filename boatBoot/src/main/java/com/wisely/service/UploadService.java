...
public class UploadService {
	 ...
	 * 存储没有存储过的数据
	 */
	public ResultVO getSmallDemo(MultipartFile file) {
		...
		List<Item> items = new ArrayList<Item>();
		 ...
		 Demometadata temp = service.findByName(demoMeta.getName());
		 if(Toolkit.notEmpty(temp)){
			 items= excelNewService.getItemData(temp);
			 serviceItem.saveAll(items);
			 re.setMessage("处理声管小样"+demoMeta.getName()+"下面的"+items.size()+"条信息完毕");
			 re.setSuccess(true);
		 }else{
			 re.setSuccess(false);
			 re.setMessage("声管小样名称不存在！");
		 }
		return re;
	}
	 ...
	 * 存储没有存储过的数据
	 */
	public ResultVO getBigDemo(MultipartFile file) {
		...
		BigDemoMetadata temp = bigDemoMetadataService.findByName(demoMeta.getName());
		if(Toolkit.notEmpty(temp)){
			 items= excelNewService.getBigItemData(temp);
			 serviceItemBig.saveAll(items);
			 re.setMessage("处理水罐大样"+demoMeta.getName()+"下面的"+items.size()+"条信息完毕");
			 re.setSuccess(true);
		 }else{
			 re.setSuccess(false);
			 re.setMessage("水罐大样名称不存在！");
		 }
		return re;
	}
	 ...
	 * 存储没有存储过的数据
	 */
	 public ResultVO getConDemo(MultipartFile file) {
		ResultVO re = new ResultVO();
		List<ItemScalePO> items = new ArrayList<ItemScalePO>();
		//先取原数据
		ScaleMataPO demoMeta = excelNewService.getScaleMetaFromExcle();
		ScaleMataPO temp = scaleMataService.findByName(demoMeta.getName());
		if(Toolkit.notEmpty(temp)){
