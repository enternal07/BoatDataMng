...
public interface BigDemoMetadataDao extends CustomRepository<BigDemoMetadata, String> {
	...
	@Query("select meta from BigDemoMetadata meta where meta.sampleName=?1 and meta.temparture=?2 and meta.press=?3 and meta.testModelName=?4 and meta.testSystemName=?5")
	List<BigDemoMetadata> findBySamplenameAndTempartureAndPressAndTestNames(String SmapleName,float Temparture ,float press,String testMname,String testSname);

	List<BigDemoMetadata> findByName(String name);
