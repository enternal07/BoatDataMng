...
public interface ItemDao extends CustomRepository<Item, String> {
	...
	@Query("select item from Item item where item.rate between ?1 and ?2 and item.smallPO.samplename =?3 and item.smallPO.backingname = ?4  and item.smallPO.temparture = ?6 and item.smallPO.press=?5  order by item.rate ASC")
	List<Item> getObjectListByContions(int rateMin,int rateMax,String sampleName,String backType,float press,float temparture);
